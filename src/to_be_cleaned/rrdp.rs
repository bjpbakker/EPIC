...
use rpki::{
    rrdp::{self, DeltaInfo, Hash, NotificationFile, Snapshot, SnapshotInfo},
    uri::Https,
};

use crate::{
    to_be_cleaned::config::Config,
    to_be_cleaned::fetch::{Fetcher, NotificationFileResponse},
    to_be_cleaned::file_ops,
    util::{self, Time},
};
                    ...
                    if snapshot.session_id() != notification_file.session_id() {
                        ...
                        match snapshot.serial().cmp(&notification_file.serial()) {
                            ...
                            std::cmp::Ordering::Equal => {
                                ...
                                debug!(
                                    "Notification file unchanged (etag not supported?), no update needed."
                                );
                                return Ok(None);
                            }
